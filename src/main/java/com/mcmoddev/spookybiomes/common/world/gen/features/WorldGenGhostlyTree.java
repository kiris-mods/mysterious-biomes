...
public class WorldGenGhostlyTree extends WorldGenAbstractTree {
    ...
    public boolean generate(@Nonnull World world, Random rand, BlockPos position) {
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= 256) {
            for (int j = position.getY(); j <= position.getY() + 1 + i; ++j) {
                ...
                BlockPos.MutableBlockPos changableBlockPos = new BlockPos.MutableBlockPos();

                for (int l = position.getX() - k; l <= position.getX() + k && flag; ++l) {
                    for (int i1 = position.getZ() - k; i1 <= position.getZ() + k && flag; ++i1) {
                        if (j >= 0 && j < 256) {
                            if (!this.isReplaceable(world, changableBlockPos.setPos(l, j, i1))) {
                                flag = false;
                            }
                        } else {
                            flag = false;
    ...
    private void placeLogAt(World worldIn, BlockPos pos) {
        this.setBlockAndNotifyAdequately(worldIn, pos, SpookyBlockObjects.GHOSTLY_LOG.getDefaultState());
    }
    ...
    private void placeLeafAt(World worldIn, BlockPos pos) {
        ...
        if (state.getBlock().isAir(state, worldIn, pos) || state.getBlock().isLeaves(state, worldIn, pos)) {
            this.setBlockAndNotifyAdequately(worldIn, pos, SpookyBlockObjects.GHOSTLY_LEAVES.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, false));
        }
    }
}
