...
public class WorldGenGhostlyTree extends WorldGenAbstractTree {
    ...
    void generateLeafNode(BlockPos pos) {
        for (int i = 0; i < this.leafDistanceLimit; ++i) {
            this.crosSection(pos.up(i), this.leafSize(i), SpookyBlockObjects.GHOSTLY_LEAVES.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, false));
        }
    }
    ...
    void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_) {
        ...
        for (int j = 0; j <= i; ++j) {
            BlockPos blockpos1 = p_175937_1_.add((0.5F + (float) j * f), (0.5F + (float) j * f1), (0.5F + j * f2));
    ...
    void generateTrunk() {
        ...
        Block block = SpookyBlockObjects.GHOSTLY_LOG;
    ...
    void generateLeafNodeBases() {
        for (FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords) {
            ...
            if (!blockpos.equals(worldgenbigtree$foliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY())) {
                this.limb(blockpos, worldgenbigtree$foliagecoordinates, SpookyBlockObjects.GHOSTLY_LOG);
            }
        }
    }
